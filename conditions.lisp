...
(define-condition null-value-error (simple-error)
  ((slot-name :initarg :slot-name :reader slot-name)
   (object :initarg :object :reader object))
  (:report (lambda (c s)
             (format s "Null value for column ~s in class ~s. Attempting to insert null value in column
that is null constrained."
                     (slot-name c) (class-name (class-of (object c))))))
  (:documentation "When inserting a row all columns with the constraint NOT NULL required a value."))

(defun null-value-error (slot-name object)
  (cerror "Add value to slot and continue."
          'null-value-error
          :slot-name slot-name
          :object object))
