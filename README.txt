...
NOTE: All tables inherited by an interface node must contain a foreign key referencing either directly, or indirectly, a key-column. An interface node is essentially a tree of relations mapped onto a database. A list of relevant tables is aggregated during compilation including both table classes listed as superclasses and tables mapped by slots of type DB-AGGREGATE-SLOT-DEFINITION. This list of tables is stored in the class slot tables, and sorted so that column slots referenced by foreign keys always precede the referring slot. If a key column is not defined, one is selected from the list of column slots.



Procedures and functions

Procedures and functions are created by the layered function GENERATE-PROCEDURE, which is called by the layered function EXECUTE. EXECUTE takes an interface node and component, and various keyword arguments, and caches the resulting procedure/function.


Refreshing the cache

To refresh the cached procedure simply set the refresh-cache keyword when invoking EXECUTE.
...
(db-connect db (insert-node) (execute (make-instance 'account) nil))
...
(db-connect db (insert-table) (execute (make-instance 'account) (find-class 'user-site)))



Updating a record:

(db-connect db (update-node) (execute <old> <new>)) 
...
(db-connect db (insert-node) (execute <object> nil))
...
For more on the merits and uses for each option, please see: https://www.postgresql.org/docs/14/ddl-constraints.html.


To delete from one table only provide the additional component (table) class, and change the context to delete-table.

(db-connect db (delete-table) (execute (make-instance 'account) (find-class 'user-site)))



Retrieving a record:

(db-connect db (retrieve-node) (execute <object> nil))

In retrieving a node the values assigned to slots are used in the "where" clause of the select statement. Joins are by default inner-joins, however tables pushed to the key parameter :OPTIONAL-JOIN of the layered function EXECUTE are reordered and joined using a left outer join. Tables pushed to the :UNION-QUERY or UNION-ALL-QUERIES key parameters, are also queried by means of a nested select expression in the "from" clause.
