...
STW-DB uses STW-META, a thin wrapper around [[https://github.com/pcostanza/contextl][CONTEXTL]]. For further information about the protocol STW-DB uses to define table classes or interface nodes please see the documentation for STW-META. With that said, the section on class definition initialization is repeated here:
...
* TODO Add support for additional options for select queries: aggregate functions such as count, max, etc, with having and group-by. The latter could be done by adding an additional interface type/context???
* TODO Add additional transaction support for procedures such as commit, rollback, and savepoint.
* TODO Revisit dates, and look at intervals.
