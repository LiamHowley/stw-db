        ...
        (iterate-extend
          ...
          (unless (slot-value slot 'primary-key)
            (when (member slot (slot-value table-class 'require-columns) :test #'eq)
              (collect-into column-name set-columns
                            (concatenate 'string "EXCLUDED." column-name) set)))
          (cond ((member slot require-columns :test #'equality)
                 (collect-into column-name required-columns))
                (t
                 (collect-into (list (set-sql-name schema domain)) args
                               "$~a" required-columns
                               `("~a" ,slot) p-controls)))
        ...
        (for= domain (slot-value slot 'domain)
              ...
              process-declared #'(lambda ()
                                   (collect-into declared-var declared-vars
                                                 ...
                                                 (car (var-var declared-var)) vars
                                                 (var-param declared-var) out-args
                                                 nil out-values)))

        ;; declared-vars are variables that are declared in the transaction so as to be
        ;; assigned a value and to be returned. These are typically but not exclusively
        ;; default values or serial values generated by the database.
        (when (and (or (eq slot-value-p :default)
                   ...
                   (if require-columns
                       ...
                       (make-component
                        :sql (format nil
                                     ...
                                     (set-sql-name schema table)
                                     columns
                                     (format nil "VALUES (~{~a~^, ~})" required-columns)
                                     returning-columns
                                     vars)
                        :declarations declared-vars
